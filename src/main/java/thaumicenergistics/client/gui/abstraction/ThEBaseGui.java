	...
	implements IWidgetHost
{
	...
	private final boolean addTooltipFromButtons( final int mouseX, final int mouseY )
	{
		...
		for( Object obj : this.buttonList )
		{
			...
			{
				// Cast
				ThEGuiButtonBase currentButton = (ThEGuiButtonBase)obj;
	...
	{
		if( mouseButton != ThEGuiHelper.MOUSE_BUTTON_LEFT )
		{
			// Mouse over button?
	...
	protected final void drawAEToolAndUpgradeSlots( final float alpha, final int mouseX, final int mouseY )
	{
		...
		for( int i = 0; i < this.inventorySlots.inventorySlots.size(); i++ )
		{
			...
			{
				// Draw background
				this.drawTexturedModalRect( this.guiLeft + slot.xDisplayPosition, this.guiTop + slot.yDisplayPosition, ThEBaseGui.upgradeU,
					ThEBaseGui.upgradeV, ThEBaseGui.upgradeWidth, ThEBaseGui.upgradeHeight );
			}
		}
	}
	...
	{
		this.onButtonClicked( button, ThEGuiHelper.MOUSE_BUTTON_LEFT );
	}
