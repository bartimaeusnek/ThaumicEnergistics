	...
	extends ThEBaseGui
{
	...
	public ThEScrollbarGui( final Container container )
	{
		super( container );
	...
	public void drawScreen( final int mouseX, final int mouseY, final float mouseBtn )
	{
		...
		if( this.isScrollBarHeld )
		{
			...
			if( Mouse.isButtonDown( ThEGuiHelper.MOUSE_BUTTON_LEFT ) )
			{
				// Has the Y changed?
	...
	public void initGui()
	{
		...
		this.aeGuiBridge = new AEBaseGui( this.inventorySlots )
		{
			...
			{
				ThEScrollbarGui.this.mc.getTextureManager().bindTexture( new ResourceLocation( base, "textures/" + file ) );
			}
